...
public class EditorControllerTest {
     ...
     */
    @Test
    @DisplayName("Step button logs to console")
    ...
    void testSetCodeProgrammatically(FxRobot robot) {
        ...
        // Eseguiamo l'aggiornamento nel thread JavaFX (EDT) per evitare eccezioni di concorrenza
        robot.interact(() -> {
     ...
     */
    @Test
    @DisplayName("Initialization should populate Memory Table and Visual Stack")
    void testMemoryAndStackInitialization(FxRobot robot) {
        // 1. Verifica Tabella Memoria (Copre initFakeData e MemoryRow)
        TableView<?> table = robot.lookup("#memoryTable").queryTableView();
        assertFalse(table.getItems().isEmpty(), "La tabella della memoria dovrebbe contenere dati fittizi all'avvio");

        // Verifica indiretta della classe MemoryRow accedendo al primo elemento
        Object firstItem = table.getItems().get(0);
        assertNotNull(firstItem, "L'elemento della tabella non dovrebbe essere null");
        assertTrue(firstItem instanceof EditorController.MemoryRow, "L'item nella tabella deve essere istanza di MemoryRow");

        // Testiamo esplicitamente i getter di MemoryRow per la coverage
        EditorController.MemoryRow row = (EditorController.MemoryRow) firstItem;
        assertNotNull(row.getAddress(), "L'indirizzo della riga di memoria non deve essere null");
        assertNotNull(row.getValue(), "Il valore della riga di memoria non deve essere null");

        // 2. Verifica Stack Visivo (Copre updateVisualStack)
        VBox stackContainer = robot.lookup("#visualStackContainer").queryAs(VBox.class);
        // initFakeData aggiunge 3 elementi (10, 55, 1024), quindi ci aspettiamo 3 nodi figli nel VBox
        assertEquals(3, stackContainer.getChildren().size(), "Lo stack visivo dovrebbe essere inizializzato con 3 elementi");
    }
}